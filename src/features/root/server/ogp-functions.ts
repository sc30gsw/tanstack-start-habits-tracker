...
export const fetchOgpData = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }): Promise<{ success: true; data: OgpData } | { success: false; error: string }> => {
    try {
...
function extractMetaContent(html: string, property: string): string | null {
  // OGタグを抽出
  const ogRegex = new RegExp(`<meta\\s+property=["']og:${property.replace('og:', '')}["']\\s+content=["']([^"']+)["']`, 'i')
