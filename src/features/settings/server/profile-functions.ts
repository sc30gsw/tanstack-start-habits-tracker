...
import { db } from '~/db'
import { habitLevels, users } from '~/db/schema'
import { calculateProfileAggregatedLevel } from '~/features/settings/utils/profile-level-utils'
...
const getAggregatedLevel = createServerFn({
  method: 'GET',
}).handler(async () => {
  const session = await auth.api.getSession(getRequest())
  ...
  const aggregated = calculateProfileAggregatedLevel(userHabitLevels)
...
export const profileDto = {
  ...
  getAggregatedLevel: typeof getAggregatedLevel
}
