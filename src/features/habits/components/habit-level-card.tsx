...
export function HabitLevelCard() {
  ...
  const { levelTab = 'overview' } = routeApi.useSearch()
  ...
  const handleTabChange = (value: string | null) => {
    if (
      value &&
      (value === 'overview' || value === 'levels' || value === 'streak' || value === 'badges')
    ) {
      navigate({
        search: (prev) => ({
          ...prev,
          levelTab: value as SearchParams['levelTab'],
        }),
      })
    }
  }
  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" withBorder>
      <Tabs value={levelTab} onChange={handleTabChange} variant="pills">
        ...
        <Tabs.Panel value="overview" pt="lg">
          <Stack gap="lg">
            ...
            <Group grow>
              ...
              <Card padding="md" radius="md" withBorder>
                <Stack gap="xs" align="center">
                  <HoursIcon size={32} color={`var(--mantine-color-${levelInfo.hours.color}-6)`} />
                  <Text size="xs" c="dimmed">
                    総時間レベル
                  </Text>
        ...
        <Tabs.Panel value="badges" pt="lg">
          <BadgeCollection
            completionLevel={levelInfo.completion.level}
            hoursLevel={levelInfo.hours.level}
          />
        </Tabs.Panel>
      </Tabs>
    </Card>
